...
function launchInstanceSetup(description, name, callback) {
  ...
  securityGroup = createSecurityGroup(description, name, function(err, data) {
    ...
    console.log('line 89 - created security group')
    // add some rules to the security group
    addSecurityRules(data, function(err, result) {
      if(err) return console.log(err);
      console.log('line 93 - added security rules')
...
/**
  ...
  Notes for tomorrow:
  - Need to delete EC2 security group first to avoid overriding it with the same name
  - Security group doesn't actually create a new rule for some reason!!

  - I have replaced the actually created groupId with a static gsecurity roup that Bitnami actually uses,
    might only work for my account.
